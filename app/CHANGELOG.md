## v0.8.21 — 2026-08-09
- **可靠的快速升级**：增量包下载、sha256 校验与文件暂存全部在应用外部完成；旧应用退出后，由独立的 macOS launchd 更新助手写入新文件、重新签名并自动启动新版本，彻底避免运行中的应用修改自身 bundle。
- **完整包可用作自动回退**：若没有匹配的增量包，更新器会下载并校验完整 DMG，再由同一独立助手原子替换整个应用；完整包和增量包均会在发布前校验版本、签名和哈希。
- **防止重复报错**：下载缓存按目标版本和 sha256 隔离，旧的残缺缓存不会再被续传或误用；增量压缩包中的路径和删除清单会先做越界校验。
- **安装位置提示**：位于桌面、下载或文稿等受保护目录时，应用会明确提示先将完整安装包装入“应用程序”文件夹，避免假装升级成功。
