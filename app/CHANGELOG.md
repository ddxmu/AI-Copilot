...
- 「拉取模型」按钮旁新增「＋ 添加模型」按钮：API 不支持在线拉取模型列表、或想用的模型不在列表里时，可手动添加。
- 弹窗填写**模型 ID**（必填，如 `gpt-3.5-turbo`）、**模型名称**（选填，如 GPT-4）、**分组名称**（选填，如 ChatGPT，相同分组的模型会在下拉列表里归到一起），点「添加模型」即加入列表并自动选中。
- 会自动校验：模型 ID 必填、重复添加会提示；保存模型配置后即可正常使用。

## v0.11.10 — 2026-09-07

### 1. 思考强度图标替换

- 聊天界面模型名旁「思考强度」的图标由 emoji 换成应用内置图标（学士帽），与整体界面风格一致。

## v0.11.9 — 2026-09-07

### 1. 新增「思考强度」调节

- **AI 设置**里每个模型可多选勾选它实际支持的强度档位：**不推理 / 轻 / 中 / 高 / 极高**；全不勾 = 该模型不支持强度调节，聊天界面就不显示强度选项。
- **聊天界面**模型名旁新增强度徽章，鼠标点一下即可切换，选择会记住并写入配置。
- 只在配置里选过档位的模型才会发送 `reasoning_effort` 参数，未配置的模型完全不受影响。
- 「不推理」用于显式关闭推理：部分推理模型（如 `gpt-5.6-luna`）带工具调用时必须选它，否则服务端会报「Function tools with reasoning_effort are not supported」错误。

## v0.11.8 — 2026-09-01

### 1. 点文件即打开「预览页面」，鼠标滚轮查看

- 「工作完成」里点条目任意位置（**不点按钮**），整个右栏立刻变成预览页面：顶部有 `← 返回`、文件名、缩放控件（按宽 / 50% / 75% / 100% / 150%）和「打开」按钮，下方内容区撑满剩余高度。
- 预览区域不再被塞在工作项里的小框中，鼠标滚轮可以直接在整个预览上滚动翻页。
- 「打开」按钮仍然用默认程序打开，「在访达显示」仍然在访达中定位，这两个按钮的点击不会误触发预览。

### 2. 修复：PDF / Office 预览被压成窄条

- 之前 PDF（含 Office 转出来的 PDF）在预览里被压成约 40px 宽的竖条，几乎看不清内容。
- 原因是 Chromium 内置 PDF 查看器默认按「整页适配」，会把所有页面整体缩到塞进预览框的高度，竖版 A4 就被压扁。
- 现在加载时强制按宽度适配，每页横向撑满预览宽度，纵向滚动即可逐页查看。

### 3. 「工作完成」列表更清爽

- 列表项不再内嵌预览区，只保留图标、文件名、路径、时间和两个操作按钮，点击整行打开预览页面。

## v0.11.7 — 2026-09-01

### 1. 「工作完成」面板支持内联文件预览

- 之前 AI 生成的文件在右栏只显示一个占位图标（图片显示小缩略图、文本显示头几百字），PDF / Word / Excel / PPT / HTML 等完全看不到内容，只能点「打开」跳到外部程序。
- 现在右栏「工作完成」里，文件会**直接内联预览**：PDF 用内置 PDF 查看器渲染、图片按面板宽度缩放、文本/代码完整显示、HTML 用沙箱 iframe 渲染、网页地址直接内嵌。
- Office 文档（`.doc` / `.docx` / `.xls` / `.xlsx` / `.ppt` / `.pptx` / `.odt` / `.ods` / `.odp`）会调用本机 LibreOffice 后台转成 PDF 再内联预览，转换结果会缓存复用；**本机未安装 LibreOffice 时会退化为提示 + 「打开」按钮**，不影响使用。
- 每条预览顶部有**文件名 + 缩放控件（按宽 / 50% / 75% / 100% / 150%）+ 「打开」按钮**，默认「按宽」即缩放到刚好填满面板宽度。
- 右侧栏宽度从 300px 增加到 380px，让预览内容有足够空间显示。
- 安全与性能：HTML 预览在沙箱 iframe 中运行（禁用脚本 / 表单 / 弹窗）；图片 ≤ 20MB、文本 ≤ 2MB、PDF ≤ 80MB 才内联预览，超大会提示并保留「打开」按钮。全部在本地完成，**不联网、不上传文件**。

## v0.11.6 — 2026-08-30

### 1. 修复：本地模型长任务未完成就中途停止

- 之前当单条回复撞上 `maxTokens` 上限被截断时，智能体会把「截断」误判成「模型说完了」，直接结束任务，导致答案表 / 长列表只输出一半就停住。
- 现在 `callApi` 会透传 `finishReason`，智能体循环在检测到 `length` 截断且无工具调用时，自动补一条「从断点继续」的系统提示续写，最多续写 3 次，避免任务突然中断。

### 2. 系统提示约束：长内容改走 write_file

- 长列表、答案表、大段代码等结构化输出现在要求先用 `write_file` 写入文件再告知路径，单条回复正文控制在 1500 字以内，从源头避免撞长度上限被截断。

### 3. 技能预检：自动激活 + 授权弹窗

- 任务触发词命中「已装技能」时，自动把该技能操作手册注入系统提示，免得弱模型不会自己调用技能工具。
- 命中「未安装的推荐技能」时，在对话入口主动弹窗请你授权安装，装完热加载即可使用（例如「浏览器自动作答」「批量重命名」「整理文件夹」等）。

## v0.11.5 — 2026-08-29

### 1. 修复：切换历史记录后看不到 AI 正在进行的工作

- AI 正在回答时（显示「思考中…」或文字正在逐字流出），如果切到别的历史记录或功能页，再切回这个会话，之前看到的思考状态和已经流出的文字都会消失，只能干等到 AI 全部回答完才重新出现。
- 原因是切换会话时聊天区会被清空并按已保存的历史重新渲染，而「思考中…」的占位和正在输入的文字属于运行中的临时状态，并不在已保存的历史里，所以切回来没有恢复。
- 现在切回正在运行 AI 的会话时，会自动恢复当时的状态：还没出字就重新显示「思考中…」（深度思考中则显示「深度思考中…」），已经出过字就把收到的内容完整显示出来，并且后续内容会接着继续流出。

## v0.11.4 — 2026-08-29

### 1. 修复：本地模型回复被拆成一行一行的窄条

- 流式输出时，每一小段文本之间被强制插入了换行，导致气泡被压成很窄的竖条，文字只能竖着往下蹦。
- 现在流式片段直接拼接，只在模型输出本身带换行时才换行，气泡恢复成正常的横向段落。

### 2. 回复逐字流出（打字机效果）

- 本地模型大约每 0.8 秒才推送 2~6 个字，之前是「一小段一次性砸出来，然后干等」，体感卡顿。
- 现在收到的文本先进入队列，再按**实测到的模型吐字速度**匀速逐字显示，视觉上是连续打字，可以直接看出模型的快慢。
- 速率是自动测量的，快模型跟得上、慢模型不空转；云端非流式（整包返回）会自动识别并一次性显示，不会逐字慢慢爬。

### 3. 等待期间显示「思考中…」

- 本地模型在开始回答前往往要先做十几秒的预处理，这段时间界面完全空白，容易以为卡死。
- 现在发出请求后立刻插入一个带跳动的占位气泡，显示 **思考中… 已等待秒数**，第一个字到达时自动撤掉。

### 4. 深度思考时显示「深度思考中…」

- 使用推理类模型（会把思维链放在 `reasoning_content` 字段里，例如 Qwen3 系列）时，进入深度思考阶段后占位气泡会自动切换成 **深度思考中…**。
- 注：思维链的正文目前只用于判断状态，不会显示出来，也不会写入对话历史。

## v0.11.3 — 2026-08-29

### 1. 修复：本地模型在长对话下长时间无响应

- 之前的自动压缩依赖「累计 token 超过阈值」才触发，而每轮新消息的累计值都从 0 开始，导致**第一条消息永远不会压缩**，会把整个对话历史原样发出去。历史一长（实测出现过约 15 万 token，而本地模型上下文窗口通常只有 32768），请求就把窗口撑爆，服务端只能把计算切成极小的块慢慢处理，界面上表现为一直转圈、迟迟不出回复。
- 现在改为**发送前就按上下文窗口裁剪**：自动算出「系统提示 + 工具定义 + 输出预留」占用的空间，只把装得下的最近若干条发出去，并保证你本轮发的消息一定保留。
- 只在配置了「上下文长度」时生效（即本地模型），云端配置行为不变。
- 裁剪只影响本次发送给模型的内容，**你的对话历史仍然完整保存**，不会被删除。

## v0.11.2 — 2026-08-29

### 1. 本地模型配置保留「API Key」输入框

- 选择本地模型（Ollama / oMLX / LM Studio / llama.cpp）时，API Key 输入框重新显示，不再隐藏。
- 部分本地推理服务同样启用了鉴权（例如 llama.cpp 的 `--api-key`、LM Studio 的鉴权设置、或在服务前加了反向代理），这类情况现在可以直接填写 Key。
- Key 仍然**不是必填**：本地服务未启用鉴权时留空即可，保存与拉取模型都不会再提示「API Key 不能为空」；一旦填写，请求会正常带上鉴权信息。

## v0.11.1 — 2026-08-29

### 1. AI 配置新增「本地模型」提供商

- 新增配置的下拉里支持 **Ollama / oMLX / LM Studio / llama.cpp** 四种本地推理服务，选中后自动填好默认地址（分别为 11434 / 8000 / 1234 / 8080 端口的 `/v1`），无需 API Key 即可保存与拉取模型。
- 本地模型拥有**独立参数区**：上下文长度、最大输出 token、温度。其中上下文长度直接决定对话压缩时机，解决了本地模型被误判成 128K 上下文、提示词撑爆却永不压缩的问题。
- 以上改动只作用于本地模型配置，云端模型配置的请求参数与鉴权方式保持原样。

### 2. 本地模型改为流式输出

- 本地模型的回复改为边生成边显示，不再需要等全部生成完才出字（此前表现为长时间转圈、界面无回复）。
- 流式异常时会自动回退到原来的整段返回方式，不会比之前更差。

## v0.11.0 — 2026-08-28

### 1. 右侧「工作完成」面板增强：在线预览 + 文件信息 + 点击打开

- 任务刚完成时，在右侧「工作完成」里内联显示「在线预览」：图片出缩略图、文本/代码/Markdown 出前几行片段、其它文件出类型图标。
- App 重新打开后（记录持久化到本地），该条显示「文件信息」（文件名、大小、修改时间、完整路径）；文件若已移动/删除会标红提示。
- 点击卡片或「打开」按钮用系统默认程序打开文件；另保留「在访达显示」定位入口。

## v0.10.30 — 2026-08-27

### 1. 语音识别（STT）新增「本地」选项

- STT 设置新增「本地（无需 Key）」模式，使用浏览器 / 系统内置 Web Speech 识别，无需第三方 API Key、不联网、不上传音频；可下拉选择识别语言（中文普通话 / 粤语、English、日本語、한국어、系统默认）。
- 语音聊天自动判定：识别到的文字自动发送，并触发当前 TTS 来源（如 MiniMax）朗读 AI 回复；远程识别路径同步改为自动发送。

### 2. STT 404 报错提示优化

- 主进程 readJsonResponse 收到 HTML 错误页（如 openresty 404）时，改为可读提示「接口返回的不是 JSON（疑似服务器错误页，如 404）。请确认接口地址是否正确、且确实指向可用的服务（接口地址通常以 /v1 结尾…）」。

## v0.10.29 — 2026-08-27

### 1. DMG 安装背景箭头修正

- 安装引导图里「拖到应用程序」的箭头，从之前错位的波浪箭头改为 AI Copilot.app 与 Applications 两个图标正中间的简洁灰色水平右箭头（按 `.DS_Store` 真实图标坐标定位，水平居中、不遮挡图标）。

### 2. AI 设置 › 记忆 新增「清空记忆」

- 记忆卡片新增红色「清空记忆」按钮（带二次确认），可一键清空全部对话记忆；原有的「查看记忆」弹窗内单条删除仍保留。

## v0.10.28 — 2026-08-27

### 1. DMG 安装背景「拉动」箭头改版

- 安装引导图里「拖到应用程序」的灰色直箭头改为更可爱的蓝色拉动箭头：左端圆环抓手（示意「拖这里」）、右端圆角三角箭头、轻微 S 波浪曲线并点缀四角星，与主题色一致，且避开图标区域。

### 2. 对话框附件图标细化（Word / Excel 专用格式）

- 附件按具体格式显示对应 logo：`.doc`→doc.png、`.docx`→DOCX.png、`.xls`→XLS.png、`.xlsx`→XLSX.png（图标取自桌面「图片」文件夹，已内置到应用资源）；`.dotx` 回退 Word 通用图标，`.csv`/`.xlsm` 回退 Excel 通用图标。

## v0.10.27 — 2026-08-27

### 1. 对话附件预览改版

- 图片附件直接显示真实缩略图；无数据时异步读取本地文件生成缩略图，加载期间用对应格式图标占位（不再显示裂图）。
- Word / Excel / PPT / PDF / TXT 等文档类型显示对应品牌 logo（图标取自桌面「图片」文件夹，已内置到应用资源）。
- 文件图标样式统一为 28×28、透明背景、暗色气泡兼容。

## v0.10.26 — 2026-08-26

### 1. 智能体技能「自动更新」开关

- 设置 › 智能体技能 新增「技能自动更新」开关：开启后自动将已安装的 GitHub 来源技能更新为最新版本。
- 保护机制：不会更新你在 AI Copilot 中编辑过的技能（安装时记录 SKILL.md 内容指纹，自动更新前比对，不一致则跳过）。
- 触发：启动 12 秒后及每 24 小时静默检查一次（受开关控制），有更新时右下角通知提示。
...
- 主路径改用 `open -a <应用名/中文名>` / `open -b <bundle id>` 真正把目标应用切到前台，不再把「应用已启动」当成「已激活」。
- 支持中文应用名与 bundle id；前台化后轮询 25×200ms ≈ 5s，要求**前台名匹配 且 目标应用拥有可操作窗口**（`readFrontWindowState`），**连续 2 次稳定**才返回成功。

### 2. 目标未成为前台 → 立即失败并禁止继续点击/输入

- `focus_app` 未能在 5 秒内确认前台时：**立即失败并置位会话停止**（不记录目标应用、清空旧坐标），本轮后续任何截图/点击/输入一律拒绝，直到新一轮指令重置。
- 每步操作前的焦点漂移检测（guard）：自动重新聚焦后仍失败同样立即中止本轮，绝不在目标未在前台时用旧坐标继续操作。

### 3. 焦点切换后旧截图坐标一律作废

- focus_app 成功后清空 `_lastPos` / `_lastCg`，返回提示要求**先重新 screenshot 再基于新截图坐标点击/输入**，绝不复用旧坐标。

### 4. AX 为空时不再阻止一次剪贴板输入

- `typeViaClipboard`：AX 返回空 role/value（未确认可编辑焦点）时**不再直接报错**，继续执行一次剪贴板 ⌘V，由粘贴后回读验证兜底判定（绝不假报成功）。

### 5. 其他

- 左键点击仍走 CGEvent（鼠标移动 → 按下 → 约 50ms → 抬起），System Events 仅一次备用，行为不变。
- ComputerUse MCP `SERVER_VERSION` 1.6.1 → 1.6.2。

## v0.10.15 — 2026-08-18

### 1. computer-use.js 以桌面版为准替换（不合并其它版本）

- `mcp-servers/computer-use.js` 整体替换为桌面源 `~/Desktop/computer-use.js` 版本，以其为准继续完善。
- 不动授权逻辑（辅助功能 / 屏幕录制权限错误提示保留）、不改 main.js、不改浏览器专用逻辑。

### 2. 左键点击：CGEvent 改为主路径，System Events 仅一次备用

- 左键点击主路径改为 CoreGraphics 真实鼠标事件：**鼠标移动 → 左键按下 → 等待约 50ms → 左键抬起**（光标真实移动、落点可靠）。
- System Events 辅助功能点击（`click at` 坐标入口）仅保留一次作为备用：只在 CGEvent 主路径失败时兜底尝试，不重复连续点击同一坐标。
- 右键 / 中键 / 双击 / 拖拽仍走 CGEvent，行为不变。

### 3. 地址栏 ⌘L 后 AX 返回空 role/value 不再直接报错

- `focus_address_bar` 发送 ⌘L 后，即使 AX 返回空 role/value（未检测到可编辑焦点），也不再直接报错——继续执行一次**剪贴板 + ⌘V 输入**，让内容仍有机会落进地址栏。
- 剪贴板 ⌘V 也失败时才会如实报失败（绝不假报成功），保留连续失败计数。

### 4. 其他

- ComputerUse MCP `SERVER_VERSION` 1.6.0 → 1.6.1。
...
- 「文件内容替换」「文件名修改」「文件格式转换」「PDF 去水印」「文件自动化」「PPT 写手（手动保存）」六个模块的**本地（非 AI）处理路径**，在开始按钮所在行的**最左侧**新增进度条，实时显示 1%–100%。
- 主进程新增 `emitModuleProgress(sender, module, index, total)`，六个处理函数逐文件回传进度（`module-progress` IPC）。
- `preload.js` 暴露 `onModuleProgress` 订阅接口。
- 渲染进程 `ensureModuleProgressEl` 在各模块 `.wizard-nav` 首位插入进度条元素，`showModuleProgress` / `hideModuleProgress` 与六个入口按钮联动；进度条含填充条与百分比数字，支持深色模式。
- AI 路径（走技能 / 智能体的分支）不显示该进度条，保持原有状态提示。

### 2. 文件格式转换默认改为本地引擎

- 「转换方式（技能）」下拉默认值由 AI 技能改为 **「本地引擎（快速，不依赖 AI）」**，打开即离线快速转换，无需等待 AI。
- 面板提示文案同步更新，说明默认本地引擎，也可切换 PDF 专项技能或 document-converter 走 AI 路径。

### 3. 修复：文件名修改出错时不再漏记结果

- `rename-files` 处理函数补回 `catch` 分支：单个文件重命名失败时正确写入 `error` 结果，进度计数不再跳号（与其余五个模块行为一致）。
