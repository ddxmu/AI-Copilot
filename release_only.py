#!/usr/bin/env python3
# release_only.py — 仅建 Release + 上传 DMG + 更新 latest.json + 推送（不删除任何文件）
# 用法：GITHUB_TOKEN=xxx python3 release_only.py
import os, sys, json, subprocess, urllib.request, urllib.error, urllib.parse, hashlib, datetime
...
def extract_notes(version):
    chlog = open(os.path.join(APP_SRC, 'CHANGELOG.md'), encoding='utf-8').read()
...
def main():
    ...
    pkg = json.load(open(os.path.join(APP_SRC, 'package.json'), encoding='utf-8'))
    version = pkg['version']
    tag = 'v' + version
    ...
    run(['git', '-C', ROOT, 'commit', '-m', f'release: v{version} (latest.json)'])
